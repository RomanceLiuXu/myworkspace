...
<head>
    ...
    <script>
        $(function(){
            ...
            $("#pagination").bootstrapPaginator({
				...
				pageUrl: function(type, page, current){
                     return "<%=basePath%>backend/product/findAll?pageIndex="+page; 
             },
        	...
        	fields: { 
        		name: {
        			validators: {
    					...
    					remote: {
    						url:'<%=basePath%>/backend/product/checkName',
    						message: '商品名称已存在',
    					}
        			},
        		},
        ...
        $.ajax({
        	type:"post",
        	url:"<%=basePath%>backend/product/showProduct",
        	data:"id="+id,
        	success:function(result){
        		console.log(result);
        		$("#pro-num").val(result.id);
        		$("#pro-name").val(result.name);
        		$("#pro-price").val(result.price);
        		$("#img2").attr("src",result.img);
        	}
        });
        }
        function deleteProduct(id){
        	layer.confirm('您确定要删除该商品吗?',{btn: ['确定', '取消'],title:"删除商品类型"}, function(){
            $.ajax({
            	type:"post",
            	url:"<%=basePath%>backend/product/deleteProduct",
            	data:"id="+id,
            	success:function(result){
...
<body>
                ...
                <table class="table table-bordered table-hover" style='text-align: center;'>
                    ...
                    <c:forEach items="${pageInfo.list }" var="p">
                        <tr>
                            ...
                            <c:if test="${p.productType.status==1 }">
                            <td>有效商品</td>
                            </c:if>
                            <c:if test="${p.productType.status==0 }">
                            <td>禁用商品</td>
                            </c:if>
                            <td class="text-center">
                                <input type="button" class="btn btn-warning btn-sm doProModify" value="修改" onclick="showProduct(${p.id })">
                                <input type="button" class="btn btn-warning btn-sm doProDelete" value="删除" onclick="deleteProduct(${p.id })">
                            </td>
                        </tr>
                      </c:forEach>
                    </tbody>
    ...
    <div class="modal fade" tabindex="-1" id="Product">
        ...
        <div class="modal-dialog modal-lg">
            <!-- 内容声明 -->
            <form  id="addform" action="<%=basePath %>backend/product/addProduct" class="form-horizontal" method="post" enctype="multipart/form-data">
    ...
    <div class="modal fade" tabindex="-1" id="myProduct">
        ...
        <div class="modal-dialog modal-lg">
            <!-- 内容声明 -->
           <form action="<%=basePath %>backend/product/addProduct" class="form-horizontal" method="post" enctype="multipart/form-data">
            <div class="modal-content">
                <!-- 头部、主体、脚注 -->
                <div class="modal-header">
                    <button class="close" data-dismiss="modal">&times;</button>
